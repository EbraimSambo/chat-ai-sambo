...
import {
  MessageSquarePlus, MessageSquare, Archive, BookOpen,
  Crown, PanelLeftClose, PanelLeftOpen, X, Loader2,
} from "lucide-react";
...
export function ChatSidebar({ collapsed, onToggle, onMobileClose }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "flex flex-col h-full bg-[#1a0f2e] border-r border-white/10 transition-all duration-300 w-64 md:w-auto",
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-4 border-b border-white/10">
        ...
        <button onClick={onMobileClose} className="md:hidden text-white/60 hover:text-white transition-colors ml-auto">
          <X size={18} />
        </button>
      </div>
      ...
      <div className={cn("px-3 mb-2", collapsed && "md:hidden")}>
        ...
        <NavItem href="/" icon={<MessageSquare size={15} />} label="Chat" active={pathname === "/"} />
        <NavItem href="/" icon={<Archive size={15} />} label="Arquivados" active={false} />
        <NavItem href="/" icon={<BookOpen size={15} />} label="Biblioteca" active={false} />
      </div>
...
function ChatHistoryItem({ chat, active, onNavigate }: { chat: Chat; active: boolean; onNavigate: () => void }) {
  return (
    <Link
      href={`/chat/${chat.id}`}
      onClick={onNavigate}
